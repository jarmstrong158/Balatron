...
Architecture:
    Input (833) → Shared Trunk (768 → 768 → 512)
...
class BalatronNetwork(nn.Module):
    ...
    Forward pass:
        state_vector (833) → trunk (512) → {policy_head, value_head}
    ...
    def forward(self, state: torch.Tensor, head_idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            state: input state vector, shape (batch, 833)
